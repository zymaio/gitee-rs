...
pub async fn dispatch_tool_call(
    ...
    match tool_name {
        // Issues
        "list_issues" => handle_list_issues(client).await.map_err(|e| e.to_string()),
        "get_issue_detail" => handle_get_issue_detail(client, arguments).await.map_err(|e| e.to_string()),
        ...
        // Pull Requests
        "list_pulls" => handle_list_pulls(client, arguments).await.map_err(|e| e.to_string()),
        ...
        "close_pull" => handle_close_pull(client, arguments).await.map_err(|e| e.to_string()),
        ...
        "search_repositories" => handle_search_repositories(client, arguments).await.map_err(|e| e.to_string()),
        ...
        // Notifications
        "list_notifications" => handle_list_notifications(client).await.map_err(|e| e.to_string()),
        ...
        _ => Err(format!("Method not found: {}", tool_name)),
    }
}